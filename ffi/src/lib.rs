...
fn set_error_and_return<T>(message: String) -> *const T {
    ffi_core::error::set_last_err_msg(&message);
    std::ptr::null()
}

#[allow(clippy::needless_pass_by_value)]
fn move_string_to_buffer(string: String, buffer: *mut c_char, length: usize) -> i32 {
    ffi_core::error::clear_last_err_msg();

    if buffer.is_null() {
        ffi_core::error::set_last_err_msg(
    ...
    unsafe {
        ...
        if string.len() >= buffer_internal.len() {
            ...
            return ffi_core::try_or_set_error!((string.len() + 1).try_into(), -1);
        }
    ...
    ffi_core::try_or_set_error!(string.len().try_into(), -1)
}
