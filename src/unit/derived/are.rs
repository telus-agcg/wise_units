use classification::Classification;
pub use dimension::Dimension;
use property::Property;
use unit::{Definition, Unit, UnitType};

#[derive(Debug, Default)]
pub struct Are;

impl Unit for Are {
    fn classification(&self) -> Classification { Classification::ISO1000 }
    fn definition(&self) -> Definition { Definition::new(100.0, "m2") }
    fn dim(&self) -> Dimension { Dimension::Length }
    fn is_arbitrary(&self) -> bool { false }
    fn is_metric(&self) -> bool { true }
    fn is_special(&self) -> bool { false }
    fn names(&self) -> Vec<String> { vec!["are".to_string()] }
    fn primary_code(&self) -> String { "ar".to_string() }
    fn print_symbol(&self) -> Option<String> { Some("a".to_string()) }
    fn property(&self) -> Property { Property::Area }
    fn secondary_code(&self) -> String { "AR".to_string() }
    fn unit_type(&self) -> UnitType { UnitType::Derived }
}
