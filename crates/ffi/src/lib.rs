...
fn move_string_to_buffer(string: String, buffer: *mut c_char, length: usize) -> i32 {
    core::error::clear_last_err_msg();

    if buffer.is_null() {
        core::error::set_last_err_msg(
    ...
    unsafe {
        ...
        if string.len() >= buffer_internal.len() {
            ...
            return core::try_or_set_error!((string.len() + 1).try_into(), -1);
        }
    ...
    core::try_or_set_error!(string.len().try_into(), -1)
}
