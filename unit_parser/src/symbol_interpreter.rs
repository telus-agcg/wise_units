...
impl SymbolInterpreter {
    pub fn interpret(&self, pairs: Pairs<Rule>) -> Result<Symbol, Error> {
        let symbol = self.visit_pairs(pairs)?;

        Ok(symbol)
    }

    fn visit_pairs(&self, pairs: Pairs<Rule>) -> Result<Symbol, Error> {
        let mut symbol = Symbol::new();

        for pair in pairs {
            match pair.as_rule() {
                Rule::symbol => {
                    symbol = self.visit_symbol(pair)?;
                }
                _ => {
    ...
    fn visit_symbol(&self, pair: Pair<Rule>) -> Result<Symbol, Error> {
        ...
        for inner_pair in pair.into_inner() {
            match inner_pair.as_rule() {
                Rule::pri_prefix => {
                    let prefix = self.visit_prefix(inner_pair)?;

                    symbol.prefix = Some(prefix);
                }
                Rule::pri_atom => {
                    let atom = self.visit_atom(inner_pair)?;
    ...
    fn visit_atom(&self, pair: Pair<Rule>) -> Result<Atom, Error> {
        println!("VISIT ATOM: {:#?}", &pair);
        println!("VISIT ATOM string: {}", pair.as_str());
        ...
        for inner_pair in pair.into_inner() {
            let atom_value = match inner_pair.as_rule() {
                ...
                _ => return Err(Error::UnknownUnitString(inner_pair.as_str().to_string())),
            };

            atom = Some(atom_value);
        }

        println!("ATOM: {:#?}", &atom);

        Ok(atom.expect("Atom must have value if token was parsed"))
    }

    fn visit_prefix(&self, pair: Pair<Rule>) -> Result<Prefix, Error> {
        let mut prefix: Option<Prefix> = None;

        for inner_pair in pair.into_inner() {
            let prefix_value = match inner_pair.as_rule() {
                ...
                _                                           => {
                    return Err(Error::UnknownUnitString(inner_pair.as_str().to_string()))
                }
            };
        ...
        Ok(prefix.expect("Prefix must have value if token was parsed"))
    }
}
