use crate::{field_eq::FieldEq, measurement::Measurement};
...
mod tests {
    ...
    fn validate_field_eq() {
        let measurement = Measurement::try_new(1.0, "ar").unwrap();
        let other = Measurement::try_new(1.0, "ar").unwrap();
        assert!(measurement.field_eq(&other));

        let measurement = Measurement::try_new(1.0, "ar").unwrap();
        let other = Measurement::try_new(1.0, "har").unwrap();
        assert!(!measurement.field_eq(&other));

        let measurement = Measurement::try_new(1.0, "100ar").unwrap();
        let other = Measurement::try_new(1.0, "har").unwrap();
        assert!(!measurement.field_eq(&other));

        let measurement = Measurement::try_new(100.0, "ar").unwrap();
        let other = Measurement::try_new(1.0, "har").unwrap();
        assert!(!measurement.field_eq(&other));
    }
}
