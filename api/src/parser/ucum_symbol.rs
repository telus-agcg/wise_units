...
pub trait UcumSymbol: Send + Sync + Copy + Clone {
    ...
    /// A `UcumSymbol`'s `definition_value` is the scalar value of how many unit this symbol can be
    /// expressed in. For example, an `[in_i]` (international inch) is defined as 2.54 cm; in that
    /// case, the `definition_value` for `[in_i]` is `2.54`.
    ///
